...
if( get_option( 'anyfont-limit-cache-size' ) && get_option( 'anyfont_cache_last_checked' ) < ( time()+( 24*60*60 ) ) ){
	if(is_readable(ANYFONT_CACHE)){
		$cachesize = anyfont_check_cache_size();
		$cache_limit = 1048576*(int)get_option('anyfont-cache-size-limit');
		if((int)$cachesize > (int)$cache_limit){
...
function anyfont_check_cache_size(){
	$cachedir = opendir( ANYFONT_CACHE );
	$cachesize = 0;
	while ($curfile = readdir($cachedir)) {
		if(!is_dir($curfile)){
...
* @return mixed
*/
function anyfont_clean_htaccess(){
	...
	if(get_option('anyfont_use_htaccess') === 'on'){
		...
		delete_option('anyfont_use_htaccess');
	}
}
...
function anyfont_install(){
	...
	add_option("anyfont_styles", array());
...
// 		$final_styles = anyfont_migrate_settings($new_styles);
		...
		$styles = maybe_unserialize(get_option("anyfont_styles"));
		if(!is_array($styles)){
			$final_styles = array(
				"image" => array(),
				"css" => array()
...
function _anyfont_edit_styles($preview=false){
	...
	$style = maybe_unserialize(get_option('anyfont_styles'));
	switch($type){
...
function anyfont_write_styles($styles){
	...
	ksort($styles);
	update_option("anyfont_styles", $styles);
...
function anyfont_replace_title($title){
	...
	if(!in_the_loop() || is_feed()){
		...
		if(!$dont_replace_title && $title_replace && !in_array("wp-admin", $uri)){
			 $all_styles = maybe_unserialize(get_option('anyfont_styles'));
			 if(is_array($all_styles)){
				$style = get_option($option_style);
				if(!isset($all_styles['image'][$style])){
...
function anyfont_replace_tag_title($title){
	...
	if(get_option('anyfont_tag_title') && !in_array("wp-admin", $uri)){
		remove_filter("single_tag_title", "anyfont_replace_tag_title", 10, 2);
		 $style = (string)get_option('anyfont_tag_title_style');
		 $urltitle = (string)urlencode((string)$title);
...
//     array_push($buttons, "fontlist");
    return $buttons;
}

function anyfont_mce_css() {
	echo '<link rel="stylesheet" href="'.get_option("siteurl").'/?afwf=editorcss" type="text/css" media="all">';
}
...
function anyfont_css_webfonts($fontfamily){
	$return = "";
// 	$font = !file_exists(ANYFONT_FONTDIR."/$fontname.ttf") ? "$fontname.otf" : "$fontname.ttf";
	require_once(ANYFONT_LIBDIR."/class.admin.php");
	$admin = new anyfontAdmin(false);
...
function anyfont_css_fontface($fontfamily, $filename, $type){
	...
	return "@font-face{font-family: '$fontfamily'; src: url('$url.eot'); src: local('☺'), url('$url.woff') format('woff'){$truetype}{$svg}; $weight; $style }";
}
...
function anyfont_using_permalinks(){
	...
	if(get_option('anyfont_disable_prettylinks') != 'on' && is_object($wp_rewrite) && method_exists($wp_rewrite, 'using_mod_rewrite_permalinks')){
		return $wp_rewrite->using_mod_rewrite_permalinks();
	}
	return false;
}

function anyfont_mce_lists(){

	$admin = new anyfontAdmin(false);

	$fonts =$admin->readFontDir(true);
...
// 		}
		$url = anyfont_using_permalinks() ? ANYFONT_IMAGE_URL."admin-small/".urlencode($fontfilename)."/".urlencode($displayname).".png" : ANYFONT_IMAGE_URL."admin-small&txt=".urlencode($fontfilename)."&displaytext=".urlencode($displayname);//.urlencode($displayname);
		$fonts[$displayname]['class_name'] = str_replace(" ", "_", $displayname);
		$fonts[$displayname]['image'] = "<img src=\"$url\" alt=\"$displayname\" title=\"$displayname\"/>";

	}
	echo '<script type="text/javascript">';
	echo 'AF_Data = {};';
	echo 'AF_Data.fontlist ='. json_encode($fonts);
	echo '</script>';
}

function anyfont_css_output(){
	...
	$font = str_replace(".css", "", urldecode($req_vars[($sn+1)]));

// 	$admin = new anyfontAdmin(false);

// 	$fonts =$admin->readFontDir(true);
	header("Content-Type:text/css");
	echo anyfont_css_webfonts($font).".".str_replace(" ", "_", $font)."{font-family:".$font.";}";
	remove_all_actions("shutdown");
	exit(0);
}

function anyfont_post_fonts($content){

	global $css_fonts;
	!is_array($css_fonts) ? $css_fonts = array() : 0;
	$regexp = "/class=\"anyfont\s(.+)\">/";
	if(preg_match_all($regexp, $content, $matches)){
		if(is_array($matches[1])){
			foreach($matches[1] as $match){
				$font = preg_replace('/">.*/',"",$match);
				if(!in_array($font, $css_fonts)){
					array_push($css_fonts, $font);
				}
			}
		} else {
			$font = preg_replace('/">.*/',"",$matches[1]);
			if(!in_array($font, $css_fonts)){
				array_push($css_fonts, $font);
			}
		}
		if(!defined("WP_ADMIN")){
			add_action("wp_footer", "anyfont_post_css");
		}
	}
	set_transient("anyfont_editor_fontlist", $css_fonts, 180);
	return $content;
}

function anyfont_editor_fonts($css){
	return $css. ",".get_option("siteurl")."/?afwf=mcebodycss";
}

function anyfont_editor_css(){
	$css_fonts = get_transient("anyfont_editor_fontlist");
	$styles = "";
	foreach($css_fonts as $font){
		$fontname = str_replace("_", " ", $font);
		$styles .= anyfont_css_webfonts($fontname)." .$font{font-family:\"".$fontname."\";} ";
	}
	remove_all_actions("shutdown");
	header('Content-Encoding: deflate');
	echo gzdeflate($styles,3);
	exit(0);
}

function anyfont_mce_button_css(){
	$admin = new anyfontAdmin(false);
...
// 		}
		if($fontdetail["webfont"] == "Yes"){
			$font = str_replace(" ", "_", $displayname);
			$styles .= "span.mceIcon.mce_$font{background:transparent url('".get_option("siteurl")."/images/admin-small/".urlencode($fontdetail['filename'])."/aA.png') no-repeat 50% 50%;}\n";
		}
	}
	header('Content-Encoding: deflate');
	echo gzdeflate($styles,3);
...
function anyfont_post_css(){
	...
	foreach($css_fonts as $font){
		$fontname = str_replace("_", " ", $font);
		$style .= anyfont_css_webfonts($fontname)." .$font{font-family:\"".$fontname."\";} ";
	}
	$style .= "</style>";
	echo $style;
}

function anyfont_insert_free_account_footer(){
	require_once(ANYFONT_LIBDIR."/class.admin.php");
	$admin = new anyfontAdmin(false);
	$status = $admin->checkKey("return");
	if(isset($status['footer_message'])){
		echo $status['footer_message'];
	}
}
...
function anyfont_init(){
	add_action('admin_print_styles', 'anyfont_scripts', 100);
}
function anyfont_scripts(){
	echo '<script type="text/javascript" src="'.get_option('siteurl').'/wp-includes/js/prototype.js"></script>';
	echo '<script type="text/javascript" src="'.get_option('siteurl').'/wp-includes/js/scriptaculous/scriptaculous.js"></script>';
}
add_action("setup_theme", "anyfont_define_url");
add_action("admin_head", "anyfont_mce_lists");
...
if((isset($_REQUEST['imgstyle']) && isset($_REQUEST['txt'])) || preg_match("/\/images\/(.*)\.png/", $_SERVER['REQUEST_URI'])){
...
} else if((isset($_REQUEST['afcss']) && isset($_REQUEST['font'])) || preg_match("/\/af-css\/(.*)\.css/", $_SERVER['REQUEST_URI'])){
	if(anyfont_ini_get_bool('display_errors')){
		ini_set('display_errors', 0);
	}
	ob_start();
	remove_all_actions("send_headers");
	remove_all_actions("get_header");
	add_action('plugins_loaded', 'anyfont_css_output');

} else if(isset($_REQUEST['afwf'])){
	if(anyfont_ini_get_bool('display_errors')){
		ini_set('display_errors', 0);
	}
	ob_start();
	...
	remove_all_actions("get_header");
	header("Content-Type: text/css");
	switch($_REQUEST['afwf']){

		case "editorcss":
			anyfont_mce_button_css();
			break;
	...
	add_filter("the_content", "anyfont_post_fonts");
	add_filter("the_excerpt", "anyfont_post_fonts");
	add_filter( 'mce_css', 'anyfont_editor_fonts' );
	...
	if(defined('WP_ADMIN') && get_option('anyfont_enable_tinymce') != false){
		...
		add_action('admin_head',  'anyfont_mce_css');
		add_filter("the_editor_content", "anyfont_post_fonts");

	}
	if(defined('WP_ADMIN') && get_option("anyfont_init_error") != false){
		add_action( 'admin_notices', 'anyfont_warning_msg');
		add_action('admin_init', "anyfont_init");
	}
	...
	add_action("shutdown", "anyfont_insert_free_account_footer");
	if(defined('WP_ADMIN')){
...
add_action( 'admin_bar_menu', 'anyfont_admin_bar', 4000 );

function anyfont_admin_bar($wp_admin_bar){
	$wp_admin_bar->add_menu( array( 'parent' => 'appearance', 'id' => 'anyfont_change_fonts', 'title' => __('Change Fonts'), 'href' => admin_url('admin.php?page=anyfont-settings') ) );
	$wp_admin_bar->add_menu( array( 'parent' => 'appearance', 'id' => 'anyfont_upload_fonts', 'title' => __('Upload New Fonts'), 'href' => admin_url('admin.php?page=anyfont-fonts') ) );
	$wp_admin_bar->add_menu( array( 'parent' => 'appearance', 'id' => 'anyfont_font_styles', 'title' => __('Edit Font Styles'), 'href' => admin_url('admin.php?page=anyfont-styles') ) );
}