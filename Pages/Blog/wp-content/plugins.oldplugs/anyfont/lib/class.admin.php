...
class anyfontAdmin {
	...
	function get_page(){
		...
		$this->checkKey();
		if(isset($this->fs_status['result']) && $this->fs_status['result'] == "success"){
	...
	function getSettings(){
		...
		if(function_exists('get_post_types')){
			...
			foreach($post_types as $type){
				if($type !== 'attachment' && $type !== 'revision' && $type !== 'nav_menu_item' && $type !== "wooframework"){
		...
		$tpl->assign("KEY_STATUS", isset($this->fs_status['msg']) ? $this->fs_status['msg'] : "");
		$tpl->assign("FONTSERV_EXTRA_SETTINGS", isset($this->fs_status['extraSettings']) ? $this->fs_status['extraSettings'] ? $tpl->fetchParsed("fontserv_settings") : "" : "");
		...
		$tpl->assign("CACHE_MAX_SIZE_HELP", isset($this->helptext['anyfont-cache-size-limit']) ? $this->helptext['anyfont-cache-size-limit'] : "");
	...
	function checkKey($echo=false){
		$status = get_option("fontserv_account");
		$time = mktime();
		$check = (isset($this->page) && $this->page == "settings") ? 30 : (60*60*24);
		if(!is_array($status) || ($time-$check) > $status['last_checked']){
			...
			$status = $server->checkKey($key, get_option("siteurl"));
			$status['last_checked'] = mktime();
			update_option("fontserv_account", $status);
		}
		$this->fs_status = $status;
		if($echo === "return"){
		...
		if($this->fontdir){
			...
			while ($filename = readdir($dir)) {
				...
				if ($fileinfo[1] == "ttf" || $fileinfo[1] == "otf" || $fileinfo[1] == "eot" || $fileinfo[1] == "js" || $fileinfo[1] == "woff" || $fileinfo[1] == "svg") {
					...
					!isset($fontspace[$fileinfo[1]]) ? $fontspace[$fileinfo[1]] = array() : 0;
...
// 		$tpl->assign("CUFON_SIZE", $this->bytecalc($fontspace['js']['size']));
	...
	function isReplaceEnabled($tpl, $section, $assign){
		$helptext = isset($this->helptext[preg_replace("/^anyfont\_?\-?/", "", $section)]) ? $this->helptext[preg_replace("/^anyfont\_?\-?/", "", $section)] : "";
		$tpl->assign("{$assign}_HELP", $helptext);
		...
		if(!$this->fontdir){
			...
			while (false !== ($e = $dir->read())){
				if($this->is_font($e)){
					...
					$buildlist[$fontdetails[1]]["trademark"] = isset($fontdetails[7]) ? $fontdetails[7] : "";
					$buildlist[$fontdetails[1]]["description"] = isset($fontdetails[10]) ? nl2br($fontdetails[10]) : "";
					$buildlist[$fontdetails[1]]["version"] = isset($fontdetails[5]) ? $fontdetails[5] : "";
					$buildlist[$fontdetails[1]]["author"] = !isset($fontdetails[8]) ? (isset($fontdetails[9]) ? $fontdetails[9] : "") : $fontdetails[8];
					$buildlist[$fontdetails[1]]["url"] = isset($fontdetails[12]) ? $fontdetails[12] : "";
					$buildlist[$fontdetails[1]]["license"] = isset($fontdetails[13]) ? nl2br($fontdetails[13]) : "";
...
// 		krumo($styles);
		if(!is_array($styles)){
			$styles = array("css" => array(), "image" => array());
		}
		return $styles[$type];
	}
	...
	*/
			...
			if (!empty($name_id_dec) and empty($font_tags[$name_id_dec])){
				...
				for($l=0;$l<$string_length_dec;$l++)
				{
					if (ord($this->text[$storage_dec+$string_offset_dec+$l]) == '0') {
						...
						$font_tags[$name_id_dec] .= mb_convert_encoding(($this->text[$storage_dec+$string_offset_dec+$l]), "UTF-8", "ASCII");
					}
				}
			}
		}
	...
	function showError($err){
		add_transient('anyfont_init_error', $err, 60*60*12);
	}
}
?>